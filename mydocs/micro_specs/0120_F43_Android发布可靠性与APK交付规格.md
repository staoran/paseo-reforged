...
| 项目              | 当前事实                                                                                                                                                              | 对 F43 的影响                                                                                  |
| ----------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------- |
| 代码基线          | F42/F43 初始实施基于 `upstream/main@6016ed705`；当前 `main` 已同步上游 0.9.1 至 `e07da55f8`，并包含身份与 APK 实现提交 `cfa0db9ff`                                    | 历史实现只能作为证据和候选方案，不能整体 cherry-pick                                           |
...
| build resource monitor                       | 记录 RSS、cgroup memory、swap、临时盘和阶段耗时，定位真正的峰值进程                                           | workflow 已按 15 秒采样，构建结束记录 cgroup memory/swap 峰值与 OOM 事件  | 保留           | 作为发布门禁和故障诊断的一部分；采样只记录白名单进程名和资源数值，不记录命令行、环境变量或 secret                                                                              |
| Gradle task build cache                      | 减少重复构建的执行任务和时间，降低重复发布的失败暴露次数                                                      | APK Gradle 命令已启用 task cache；成功和失败缓存分键，尚无新 runner 实测  | 保留           | 使用稳定的 commit/Java/runner 输入 key，区分 exact hit 与 partial restore；历史 seed/hit 只证明耗时下降，峰值差异很小，仍需保留资源观测                                        |
| commit-addressed Gradle cache key            | 避免以 `github.run_id` 产生大量重复 cache，并让同一不可变 tag 能 exact hit                                    | v2 key 绑定 Reforged、Java 21、arm64、commit 和完成状态；尚无 runner 实测 | 保留           | 同 commit 的 partial cache 可供失败重试，成功后写 complete key；v1 仅作为相同身份和 ABI 的恢复来源，不让失败缓存占用成功键                                                     |
