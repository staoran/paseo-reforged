...
3. 运行目标检查，提交并发布下一 beta，验证 APK workflow
...
| 定向测试               | `npx vitest run packages/app/plugins/with-hermes-memory-budget.test.ts --bail=1`                 | 通过   | `3/3`                                                                                                                                         |
| 静态检查               | `npm run typecheck`、`npm run lint`                                                              | 通过   | typecheck exit `0`；lint `0 warnings / 0 errors`                                                                                              |
| 格式检查               | `npm run format:check:files -- ...`                                                              | 通过   | 4 个改动文件均已格式化                                                                                                                        |
| APK 发布               | GitHub Actions Android workflow                                                                  | 待执行 | 待填写                                                                                                                                        |

- 未验证项与原因：远端 APK 仍待 beta.4 tag 推送后由 EAS 优先通道验证
- 剩余风险：关闭 Metro map 会降低远端 release 的 JS 栈映射能力；AOT validator 和 WebView HTML 仍是未来可独立研究的 bundle 体积来源
...
- 状态说明：执行中
- 当前卡点：无
- 下一步唯一动作：提交构建配置并发布 `v0.7.2-beta.4`
