...
- Read First:
  ...
  - [`CLAUDE.md`](../../CLAUDE.md): 项目不可变约束与文档路由。
- Edges / Children:
...
| Client             | [`packages/client`](../../packages/client)                         | WebSocket driver、correlated RPC、SDK facade                        | protocol、WebSocket                        | reconnect、timeout、能力协商            |
| Server             | [`packages/server`](../../packages/server)                         | daemon、agent/workspace/terminal lifecycle、MCP、storage            | protocol、client、relay、provider runtimes | 状态机、文件系统、进程和兼容边界集中    |
...
| CLI                | [`packages/cli`](../../packages/cli)                               | Docker-style commands 和结构化输出                                  | client、protocol、Commander                | 自动化契约与退出错误                    |
...
- Entries:
  - Daemon bootstrap: [`packages/server/src/server/bootstrap.ts`](../../packages/server/src/server/bootstrap.ts) -> HTTP/WS、registries、AgentManager、relay、services 装配。
  ...
  - CLI: [`packages/cli/src/index.ts`](../../packages/cli/src/index.ts) -> executable；[`packages/cli/src/cli.ts`](../../packages/cli/src/cli.ts) -> command registry。
...
- Type: `flow`
- Status: `confirmed`
- Purpose: 显示 package 间主干、五张 CodeMap 的跨能力依赖和按场景下钻顺序；具体分支留给 Feature Map。

```mermaid
flowchart LR
  App["Expo App / Desktop renderer"] --> Client["@getpaseo/client"]
  CLI["Paseo CLI"] --> Client
  Client --> Protocol["@getpaseo/protocol"]
  Client --> WS["Daemon WebSocket Session"]
  Relay["Encrypted relay"] -. "optional transport" .-> WS
  WS --> Services["Agent / Workspace / Terminal services"]
  Services --> Providers["Agent providers"]
  Services --> Storage["PASEO_HOME + Git/worktrees"]
  Services --> Client
```
...
- Feature anchors: [`Agent / Timeline`](2026-07-20_01-47_Agent创建执行与Timeline同步功能.md)、[`Workspace / Worktree`](2026-07-20_01-47_Workspace-Worktree与归档恢复功能.md)、[`Terminal / Backpressure`](2026-07-20_09-20_Terminal-Binary-Frame与Backpressure功能.md)、[`Relay / E2EE`](2026-07-20_09-20_Relay配对E2E加密与远程连接功能.md)。
...
- Updated `2026-07-20`: persisted the confirmed five-map dependency graph and progressive reading order after a no-drift review。
- Drift check date: `2026-07-20`; drift signals are registered in [`.skills/project/CODEMAP_INDEX.md`](../../.skills/project/CODEMAP_INDEX.md)。
