...
| `v0.2.5-beta.1`  | 发布 | 原子 tag push 未自动触发三条发布 workflow，本轮需手动 dispatch          | 核对 GitHub 凭证/事件链路并在下一 beta 验证自动 tag trigger     |
| `v0.2.5-beta.1`  | 发布 | Release Notes Sync 会覆盖 macOS beta 未签名/未公证提示                  | 将警告纳入持久事实源或同步脚本，并补回归测试                    |
