...
- The E2EE layer implements `framedCiphertextV1`, which separates the authenticated payload kind from the WebSocket representation and requires an exact encrypted confirmation before application attach. It is currently enabled only by isolated validation dependencies: production clients do not advertise it and production daemons do not accept it after the hosted near-32 MiB relay gate failed with close code `1009`.
- Production relay data connections therefore remain on the legacy Base64/hybrid encrypted wire. No persisted config, CLI flag, environment variable, or ordinary `DaemonClientConfig` field can enable framed transport.
- When isolated validation explicitly enables framed transport, daemon-to-client state-sync, eligible completed tool-call, and UTF-8 file frames may use bounded raw DEFLATE level 1 before encryption. Realtime terminal/agent deltas, unknown traffic, and all client outbound frames remain identity-coded.
- `daemon.relay.transport` persists the future `ciphertextEncoding` (`auto`, `base64`, or `binary`) preference and compression enable switch, but neither setting changes the production wire while the release gate is closed. Under validation, compression policy updates apply to eligible frames immediately; representation is immutable on an existing data connection and changes on the next connection.
- Framed wire is rejected at 32 MiB, while aggregate prepared-send, receive-queue, and physical-socket reservations are bounded at 64 MiB. Parsing validates canonical Base64, authenticated lengths/codecs, bounded inflate, payload kind, and UTF-8 before delivery.
...
Every physical send path enforces a 64 MiB outbound high-water mark, including JSON broadcasts, binary terminal frames, and the encrypted relay adapter's asynchronous prepare queue. This sits above the terminal stream's 4 MiB soft backpressure threshold, leaving room for snapshot catch-up before the hard cutoff. Framed relay ciphertext has a stricter exclusive 32 MiB per-frame wire limit, and preparation reserves conservative wire capacity before asynchronous compression/encryption starts. JSON is serialized once per broadcast after sockets already at the limit are removed, then its exact byte length is checked for every remaining socket. A frame that would cross the applicable limit is not sent; that physical socket is forcibly terminated without disturbing other sockets attached to the same logical session. Multiple tabs and simultaneous direct and relay paths may legitimately share a client id.
