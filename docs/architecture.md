...
- E2EE `framedCiphertextV1` negotiation separates the authenticated payload kind from the WebSocket representation. New peers select Base64 or binary ciphertext and require an exact encrypted confirmation before application attach; either mixed-version direction falls back to the legacy Base64/hybrid wire.
- Daemon-to-client state-sync, eligible completed tool-call, and UTF-8 file frames may use bounded raw DEFLATE level 1 before encryption. Realtime terminal/agent deltas, unknown traffic, and all client outbound frames remain identity-coded.
- `daemon.relay.transport` controls `ciphertextEncoding` (`auto`, `base64`, or `binary`) and the compression enable switch. Compression policy updates apply to eligible frames immediately; representation is immutable on an existing data connection and changes on the next connection.
