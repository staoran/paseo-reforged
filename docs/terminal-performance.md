...
- **Realtime relay traffic never enters the compressor.** The framed/compression implementation is currently behind a closed production release gate, so production relay traffic remains on the legacy Base64/hybrid wire without transport compression. In isolated framed validation, terminal `Output`/`Input`/`Resize`, incremental assistant/reasoning output, non-completed tool calls, control messages, and unknown messages are classified as `realtime` at the serialization boundary and remain framed identity. Only state-sync snapshots, eligible completed tool output, and UTF-8 file chunks may attempt daemon-side compression.
- **Compression work cannot change transport order.** The encrypted relay adapter reserves capacity before asynchronous preparation and drains one FIFO through physical completion. A state-sync or `bulk-live` frame cannot be overtaken by a later realtime frame, so lower wire time must not be bought by reordering terminal or agent output.
...
- **Relay codec benchmark:** run `npx tsx scripts/benchmark-relay-frame-codec.ts --json <json> --terminal <terminal> --file <utf8-file> --tool-call <tool-call> --runs 5`. Structured terminal and tool-call inputs additionally accept `--terminal-format codex-session-jsonl` and `--tool-call-format paseo-timeline-segment`. The command reports aggregate bytes, throughput, adoption, and event-loop delay without printing paths or contents; it is an offline signal, not a substitute for concurrent terminal p95 under a shaped live relay.
- **Relay realtime validation:** `packages/server/scripts/measure-live-relay-performance.ts` explicitly injects framed support into an isolated daemon/client pair and rejects the connection before inventory, subscriptions, or sampling unless the authenticated mode is exactly the requested legacy or framed representation. This validation-only override is not available through production config, CLI, env, or ordinary client options.
...
- Relay-attached clients pay pure-JS tweetnacl encryption on the daemon main loop (`packages/relay/src/encrypted-channel.ts`). Production currently retains the legacy Base64/hybrid wire because hosted near-limit framing failed its release gate. Validation shows framed binary can avoid Base64 for both original text and binary payloads, and raw DEFLATE can use the native zlib callback for eligible daemon outbound frames while realtime traffic records zero compression attempts; those benefits are not production behavior until the gate is reopened.
