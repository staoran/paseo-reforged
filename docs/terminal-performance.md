...
- Relay-attached clients pay pure-JS tweetnacl encryption on the daemon main loop (`packages/relay/src/encrypted-channel.ts`). Framed binary avoids Base64 for both original text and binary payloads; forced framed Base64 and mixed-version traffic retain Base64 wire overhead. Raw DEFLATE uses the native zlib callback only for eligible daemon outbound frames, while realtime traffic records zero compression attempts.- `sendToClient` re-stringifies session messages per socket; only matters for multi-socket connections.
