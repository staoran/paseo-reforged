...
- **Realtime relay traffic never enters the compressor.** Terminal `Output`/`Input`/`Resize`, incremental assistant/reasoning output, non-completed tool calls, control messages, and unknown messages are classified as `realtime` at the serialization boundary and remain framed identity. Only state-sync snapshots, eligible completed tool output, and UTF-8 file chunks may attempt daemon-side compression.
...
- Relay-attached clients pay pure-JS tweetnacl encryption on the daemon main loop (`packages/relay/src/encrypted-channel.ts`). Framed binary avoids Base64 for both original text and binary payloads; forced framed Base64 and mixed-version traffic retain Base64 wire overhead. Raw DEFLATE uses the native zlib callback only for eligible daemon outbound frames, while realtime traffic records zero compression attempts.
