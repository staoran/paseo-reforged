...
The GitHub APK workflow first tries an EAS cloud build with the `production-apk` profile. That profile explicitly sets `ORG_GRADLE_PROJECT_reactNativeArchitectures=arm64-v8a`, so the cloud artifact follows the same ABI contract as the local fallback. If the cloud build fails, the workflow falls back to `eas build --local` on its Ubuntu runner; that path uses the EAS-managed Android keystore but does not consume EAS build quota, and it remains subject to the hosted runner's memory limits. Both paths disable remote release source maps, build only `arm64-v8a`, and are validated for the APK signature, package ID, version name, and sole native ABI before upload. The GitHub prerelease APK therefore supports 64-bit ARM devices, not 32-bit ARM devices or x86/x86_64 emulators.
