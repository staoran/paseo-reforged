...
| `lastMessageAt`        | `string?` (ISO 8601, nullable)           | Canonical timestamp of the latest persisted `user_message` or `assistant_message`; legacy records may omit it and records with no known message use `null`                                                                                                                                                                                                                          |
...
| `owner`                | `AgentOwner?`                            | Durable ownership; Hub Agents use `{ kind: "daemon", daemonId, executionId }`                                                                                                                                                                                                                                                                                                       |
| `hubExecutionContract` | `HubExecutionContract?`                  | Private Hub request/policy fingerprint and `prepared`/`applied` state; never projected to clients                                                                                                                                                                                                                                                                                   |

`lastMessageAt` is derived only from canonical timeline row timestamps. Tool, reasoning,
usage, status, configuration, runtime recovery, and other non-message activity may advance
`updatedAt` but never `lastMessageAt`. Active legacy records can recover the value from already
loaded durable canonical rows; stored-only records remain `null` when no canonical message time is
available. Agent snapshot and list payloads expose the field as optional and nullable so new clients
accept old daemons, while old clients can ignore the additive field. The App normalizes an omitted
...
| `approvalPolicy`   | `string?`                    | Legacy beta.5 Codex policy input, retained during compatibility reads |
| `sandboxMode`      | `string?`                    | Legacy beta.5 Codex sandbox input                                     |
| `networkAccess`    | `boolean?`                   | Legacy beta.5 Codex network input                                     |
| `webSearch`        | `boolean?`                   | Legacy beta.5 Codex web-search input                                  |
| `extra`            | `Record<string, any>?`       | Legacy provider-specific configuration                                |
| `providerOptions`  | `Record<string, JsonValue>?` | Canonical provider-native configuration                               |
| `toolPolicy`       | `ToolPolicy?`                | Exact MCP tool preapprovals                                           |
| `systemPrompt`     | `string?`                    | Custom system prompt                                                  |
| `mcpServers`       | `Record<string, any>?`       | MCP server configurations                                             |

The store preserves legacy and canonical fields together. Compatibility normalization creates a
runtime-only `resolvedProviderOptions` value; that resolved value is never serialized. Ordinary
snapshot and upsert writes run through the per-Agent mutation queue and preserve the latest Hub
execution contract rather than replacing or downgrading it.

### Nested: HubExecutionContract

| Field                  | Type                    | Description                                                  |
| ---------------------- | ----------------------- | ------------------------------------------------------------ |
| `protocolVersion`      | `1`                     | Local durable contract version                               |
| `executionFingerprint` | `string` (64 hex chars) | Hash of the complete normalized create intent                |
| `policyFingerprint`    | `string` (64 hex chars) | Hash of normalized provider options, MCP servers, and policy |
| `applicationState`     | `prepared` or `applied` | Whether policy setup is only durable or fully applied        |

The first Hub owner/config snapshot is awaited and written with `prepared`. A dedicated queued
compare-and-set changes that exact contract to `applied` before the initial prompt starts. Normal
snapshot writes cannot remove or downgrade the contract. Missing contracts are classified as
legacy; malformed or `prepared` records are isolated from load/replay, while matching `applied`
records can produce a durable policy acknowledgement.
...
- `{ type: "new-agent", config: { provider, cwd, modeId?, model?, thinkingOptionId?, archiveOnFinish?, isolation?, title?, approvalPolicy?, sandboxMode?, networkAccess?, webSearch?, extra?, providerOptions?, toolPolicy?, featureValues?, systemPrompt?, mcpServers? } }` — create a new agent

Schedule create, update, storage, and execution keep the same legacy/canonical dual-read contract as
direct Agent creation. Updates must preserve both `providerOptions` and `toolPolicy`; clients gate
those fields on the daemon's matching `server_info` features before sending a request.
