...
- **`lineHeight.diff` is the code/diff line-height axis** — it is coupled to the code-font-size control (≈ `codeFontSize * 1.5`). Do NOT use it for prose. Workspace prose line-height derives from the corresponding `workspaceFontSize` token; Routing prose through `lineHeight.diff` clips text at small code sizes. Markdown body line-height scales with content size (`Math.round(theme.fontSize.content * 1.4)`).
