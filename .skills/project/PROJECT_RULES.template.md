...
| enabled_workflows | `<可选增强模块：grill-me, task-tracking, task-decomposition, cross-task-collaboration 的子集；SDD Harness 与 cross 安全流程不可关闭>` |
...
`enabled_workflows` 只能控制可选增强模块：`grill-me`、`task-tracking`、`task-decomposition`、`cross-task-collaboration`。所有工程任务的 `sdd-riper` Harness 不能关闭；`cross` 任务的 `cross-project` 不能关闭。若项目未启用某个可选模块但任务触发其能力，必须在本文件或 `CUSTOM_SKILL_OVERRIDES.md` 指定等价回退流程；没有等价回退时升级为阻塞，不得静默跳过。
