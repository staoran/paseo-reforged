...
SDD-RIPER 是所有工程任务的主 Harness；其他能力只在对应阶段提供输入、约束或输出，不建立平行状态机。登记为全局必需依赖的 Skill 只在命中触发时检查，但一旦命中便不可降级为本地替代。

| 任务信号 | SDD 阶段 | 必需依赖或 bundled 入口 | 输出 / 缺失处理 |
| --- | --- | --- | --- |
| 任何工程任务 | 全程 | branch/PR/WIP/fixed-point 代码 Review、全库架构审计、`complex` / `cross`、高风险、任务单元不清或重审计使用 `$sdd-riper-one`；其余目标边界清楚且风险中低的任务使用 `$sdd-riper-one-light` | 完整加载所选全局 Skill；缺失时停止并提示安装 |
| 陌生项目、跨模块链路、架构影响或地图漂移 | Research / Review | `$codemap` | 只读查询在响应内返回源码证据、地形或 drift 结论；持久任务获写入授权后产出/更新 CodeMap 并登记 `CODEMAP_INDEX.md`；缺失时停止并提示安装 |
...
| 用户要求设计或改善深模块、interface 或 seam | Research / optional Innovate / Plan | `$codebase-design` | 只提供设计词汇与候选接口；已采纳决定回写 spec，不自动实施重构 |
...
1. 按根入口先读取 `PROJECT_RULES.md` 的 `skill_registry` 和 `enabled_workflows`，再按 `operating-model.md` 的任务分类选择全局 SDD Skill。`skill_registry` 必须指向 `DEPENDENCY_SKILLS.md` 或等价的唯一可发现性注册表。
2. 注册表已初始化时，从中解析当前阶段命中的 `required_dependency`、`source_scope`、`version_policy`、`missing_action`、`evidence` 和 `last_verified`。若 canonical route 已命中但注册表没有对应快照，标记 registry drift，停止该 capability 的执行并通过 `dependency-capability` 提交登记 checkpoint；不得把缺行解释为能力可用或不需要。Fresh Bootstrap 中注册表尚未初始化时，按本文件的核心能力路由和当前 runtime 可见能力核验本任务依赖；只把依赖、来源、版本策略与核验证据列入 Bootstrap 待登记项，不把当前可见性伪装成已初始化注册表。命中根入口的局部候选条件时返回其轻量分支，不展开 Bootstrap。
...
`enabled_workflows` 只能筛选可选的 bundled 增强能力：`grill-me`、`task-tracking`、`task-decomposition`、`cross-task-collaboration`。SDD-RIPER 核心 Harness 与 `cross` 的 cross-project 安全流程不能被项目配置关闭；bundled 合同文件缺失时停止并修复规则包，不临时创建替代流程。
...
- 未命中重型信号，且目标、边界、验证方式基本清楚、风险中低时，必须使用 `$sdd-riper-one-light`，保持短 checkpoint。
- 所选全局 Skill 未安装、不可加载或版本策略不满足时，停止当前任务并提示安装/更新。项目运行模型和持久化规则不能替代 Harness Skill。

### Codemap

- 只由 SDD Research/Pre-Research 的陌生度、跨模块链路、影响面和既有地图漂移信号触发。
- 小范围任务入口、依赖与事实源已经清楚时不触发 Codemap，直接读取源码和测试即可。
...
- Matt 能力只在表中精确信号命中时加载，并按 `customizations/matt-pocock-bridge.md` 接入当前 SDD 阶段；它们不接管任务分类、spec、批准、Review、Bootstrap、handoff 或 Reverse Sync。
...
- user-invoked 的 `$improve-codebase-architecture` 只有用户显式调用后才生成临时体检证据；普通自然语言架构分析由 SDD Research 处理，只有本文件 Codemap 触发信号成立时才追加 `$codemap`，不因未显式调用体检 Skill 而停止。
- 分支/PR/固定点 Review、巨大模糊任务、任务拆解、Bootstrap、项目配置和交接分别由 SDD 原生 Review、Research/Grill/tracking/decomposition、项目 Bootstrap/customization 与 `$new-chat-ready` 负责，不路由到 Matt 同类流程。
...
- 分支/PR Review、巨大模糊任务、任务拆解、Bootstrap 和 handoff 继续使用 SDD 原生能力；显式架构体检的临时报告不能取代逻辑 Spec Record、CodeMap 或 task plan。
