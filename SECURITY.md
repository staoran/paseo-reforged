...
5. The implementation can negotiate `framedCiphertextV1` in `e2ee_hello` / `e2ee_ready`. When explicitly enabled for isolated validation, the daemon selects Base64 text or raw binary ciphertext plus the common compression codecs, and the client must return an exact authenticated `e2ee_mode_confirm` before the daemon attaches the application session. A missing or mismatched confirmation closes the connection.
...
`framedCiphertextV1` is currently held behind a closed production release gate. The hosted relay rejected both Base64 and binary frames near the local 32 MiB boundary with close code `1009`, and the target Hermes release-device decode gate is not complete. Production clients therefore do not advertise framed support and production daemons do not accept it. Current production relay connections continue to use the legacy Base64/hybrid encrypted wire; no persisted setting, CLI flag, environment variable, or ordinary client option enables the validation override.
...
Compression is part of the gated framed implementation. When framed validation is explicitly enabled, compression happens before encryption and only on daemon-to-client frames classified as state synchronization, eligible completed tool output, or UTF-8 file chunks. Realtime terminal and agent-stream deltas, unknown messages, and client-to-daemon traffic remain identity-coded. The v1 encoder uses raw DEFLATE level 1 internally and adopts a candidate only when bounded size, minimum gain, and compression-ratio checks pass; codec and level are not user-selectable wire inputs. While the production framed gate is closed, production relay frames are not compressed even if `daemon.relay.transport.compression.enabled` is persisted as `true`.

Compression does not hide traffic analysis. The relay already observes ciphertext sizes and timing, and compressed sizes can reveal repetition or relative content size. Paseo does not add padding in v1, so deployments that require resistance to size correlation should keep compression disabled if the framed release gate is enabled in a future release.
...
Paseo Reforged does not currently have private vulnerability reporting enabled. Do not send Reforged-only reports to the upstream maintainer or open a public issue. If the vulnerability also affects upstream Paseo, report that upstream impact privately to hello@moboudra.com and identify it as an upstream report. A separate private reporting channel is still required for Reforged-only vulnerabilities.
