...
function mapCodexAgentMessage(item: Record<string, unknown>): AgentTimelineItem {
  ...
  return createAssistantTimelineMessage(
    ...
    parseAgentMessagePhase(item.phase),
  );
}
...
export class CodexAppServerAgentSession implements AgentSession {
  ...
  constructor(
    ...
    this.providerOptions = config.resolvedProviderOptions
      ? (config.resolvedProviderOptions as CodexProviderOptions)
      : CodexProviderOptionsSchema.parse(config.providerOptions ?? {});
    this.config = { ...config, providerOptions: this.providerOptions };
