...
type ClaudeResolvedProviderOptions = ClaudeProviderOptions & {
  env?: Record<string, string | undefined>;
};

type ClaudeAgentConfig = Omit<AgentSessionConfig, "providerOptions"> & {
  provider: "claude";
  providerOptions: ClaudeResolvedProviderOptions;
};
...
export class ClaudeAgentClient implements AgentClient {
  ...
  private assertConfig(config: AgentSessionConfig): ClaudeAgentConfig {
    ...
    const providerOptions = config.resolvedProviderOptions
      ? (config.resolvedProviderOptions as ClaudeResolvedProviderOptions)
      : ClaudeProviderOptionsSchema.parse(config.providerOptions ?? {});
    return {
      ...config,
      provider: "claude",
...
class ClaudeAgentSession implements AgentSession {
  ...
  private buildSdkEnv(): NodeJS.ProcessEnv {
    return createProviderEnv({
      baseEnv: process.env,
      runtimeSettings: this.runtimeSettings,
      overlays: [this.config.providerOptions.env, this.launchEnv],
    });
  }
