...
function normalizeReplayableTimelineEvent(
  ...
  const shouldMarkReplayableUserMessage =
    ...
    (pendingRun.start.status !== "started" || pendingRun.start.turnId === event.turnId);
  const item =
...
export class AgentManager {
  ...
  // oxlint-disable-next-line complexity -- reload preserves lifecycle, persistence, and rollback state together
  ...
  private async cascadeArchiveChildren(parentAgentId: string): Promise<void> {
    ...
    if (!records.some((record) => record.id === parentAgentId)) {
  ...
  async replaceAgentRun(
    ...
    if (pendingRun?.purpose === "edit_last_user_message" && pendingRun.start.status !== "started") {
  ...
  private async cancelAgentRunNow(agentId: string): Promise<AgentRunCancellationResult> {
    ...
    if (
      run?.kind === "foreground" &&
      run.purpose === "edit_last_user_message" &&
      run.start.status !== "started"
    ) {
