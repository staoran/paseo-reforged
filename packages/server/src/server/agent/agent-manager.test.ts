...
test("archiveAgent re-reads a child before deciding whether to cascade", async () => {
  ...
  class ChildOpensAfterCascadeListStorage extends AgentStorage {
    ...
    override async listAllMetadata(): Promise<AgentMetadataEntry[]> {
      const records = await super.listAllMetadata();
      ...
      if (parentIsArchived && staleChild) {
        const currentChild = await super.get(staleChild.id);
        if (currentChild) {
          await super.upsert({
            ...currentChild,
            labels: { ...currentChild.labels, [MOBILE_OPEN_AGENT_TAB_LABEL]: "true" },
          });
        }
      }
      return records;
