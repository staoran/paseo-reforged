...
interface SupportedMutableConfigPatch {
  relay?: {
    enabled?: boolean;
    transport?: NonNullable<MutableDaemonConfig["relay"]>["transport"];
  };
...
// oxlint-disable-next-line complexity -- Patch compatibility requires field-by-field filtering
function pickSupportedPatchFields(patch: MutableDaemonConfigPatch): SupportedMutableConfigPatch {
  return {
    ...(patch.relay?.enabled !== undefined || patch.relay?.transport !== undefined
      ? {
          relay: {
            ...(patch.relay?.enabled !== undefined ? { enabled: patch.relay.enabled } : {}),
            ...(patch.relay?.transport !== undefined ? { transport: patch.relay.transport } : {}),
          },
        }
      : {}),
...
function mergeMutableDaemonPatch(
  ...
  // Fork(0091/0095): transport 与嵌套 compression 字段独立于 enabled 门控合并保留。
  if (patch.relay?.transport !== undefined) {
    const patchTransport = patch.relay.transport;
    next.relay = {
      ...next.relay,
      transport: {
        ...next.relay?.transport,
        ...patchTransport,
        ...(patchTransport.compression !== undefined
          ? {
              compression: {
                ...next.relay?.transport?.compression,
                ...patchTransport.compression,
              },
            }
          : {}),
      },
