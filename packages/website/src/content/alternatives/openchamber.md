...
OpenChamber packages its web interface for iOS and Android with Capacitor. The interface runs inside WKWebView or Android WebView, with native integrations for notifications, secure storage, QR pairing, and widgets. Paseo Reforged's mobile client is built with React Native; Reforged currently distributes Android APKs, while new store listings are not published yet.
