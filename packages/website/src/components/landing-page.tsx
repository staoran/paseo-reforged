...
import { getDownloadOptions, useDetectedPlatform, TerminalIcon } from "~/downloads";
...
const SERVER_INSTALL_FOOTNOTE = (
  <>Requires Nix with flakes enabled. The command starts the Paseo Reforged daemon and CLI.</>
);
...
function ServerInstallButton() {
  return (
    <CommandDialog
      ...
      description="For headless machines you want to connect to from Paseo Reforged. The desktop app already includes a built-in daemon."
      command="nix run github:staoran/paseo-reforged"
    ...
    >
      ...
      <div className="space-y-6">
        ...
        <FAQItem question="Do I need the desktop app?">
          ...
          and connect from the CLI, desktop app, or Android app. The desktop app just bundles the
          daemon with a UI.
        </FAQItem>
...
function _HubSection() {
  return (
    <FeatureSection
      title="Upstream Paseo Hub"
      badge="New"
      description="An optional service maintained by upstream Paseo. It adds GitHub, Slack, and Discord triggers above compatible daemons; it is not a Paseo Reforged release or hosted service."
    >
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
      ...
      >
        View the upstream Hub overview
      </a>
    </FeatureSection>
  );
}

function _SponsorCTA() {
  return (
    <motion.div
      initial={FADE_IN_UP}
      whileInView={FADE_IN}
