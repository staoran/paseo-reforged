...
export type TransportTarget = LocalTransportTarget | SshTransportTarget | WebSocketTransportTarget;
...
export interface TransportEndpoint {
  ...
  webSocketOptions?: { headers?: Record<string, string>; protocols?: string[] };
}
...
const HTTP_HEADER_NAME_PATTERN = new RegExp("^[!#$%&'+\\-.^`|~0-9A-Za-z]+$");

/** Fork(0102): 校验并归一化 fork 的 paseo+local WebSocket 目标(头名/换行注入防护)。 */
...
export function parseTransportTarget(value: unknown): TransportTarget {
  ...
  if (value.transportType === "websocket") {
    if (typeof value.url !== "string") {
      throw new Error("WebSocket transport URL is required.");
    }
...
async function resolveTransportEndpoint(target: TransportTarget): Promise<TransportEndpoint> {
  ...
  if (target.transportType === "websocket") {
    ...
    return {
      ...
      webSocketOptions: {
        ...
        ...(normalized.protocols ? { protocols: normalized.protocols } : {}),
      },
    };
  }
  return {
...
export function createLocalTransportManager(
  ...
  async function connectSession(session: Session): Promise<void> {
    ...
    try {
      ws = deps.createWebSocket(endpoint.url, endpoint.webSocketOptions);
    } catch (error) {
      failOpeningSession(
        session,
        `Failed to connect to ${describeTransportTarget(session.target)}: ${getErrorMessage(error)}`,
