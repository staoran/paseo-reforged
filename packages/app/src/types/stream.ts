...
function areUserMessagesEquivalent(left: UserMessageItem, right: UserMessageItem): boolean {
  return (
    left.id === right.id &&
...
function produceUserMessage(
  ...
  const merged = createUserMessage({
    ...
    replayKind: incoming.replayKind ?? existing.replayKind,
  });
  if (areUserMessagesEquivalent(existing, merged)) {
...
function applyCanonicalUserMessageEvent(params: {
  ...
  const canonical = createUserMessage({
    ...
    timelineCursor,
    replayKind: event.item.replayKind,
