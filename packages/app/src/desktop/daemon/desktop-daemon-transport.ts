...
export function buildDesktopDaemonTransportUrl(target: DesktopDaemonTransportTarget): string {
  ...
  } else if (target.transportType !== "websocket") {
...
function parseDesktopDaemonTransportUrl(url: string): DesktopDaemonTransportTarget {
  ...
  if (parsed.protocol !== DESKTOP_TRANSPORT_SCHEME && parsed.protocol !== "paseo+local:") {
...
export function createDesktopWebSocketTransportFactory(
  ...
  const effectiveRpc = rpc ?? defaultWebSocketDaemonTransportRpc;
  return (options) => {
    ...
    const target: DesktopDaemonTransportTarget = {
      ...
      ...(options.protocols && options.protocols.length > 0
    ...
    void (async () => {
      try {
        const cleanup = await effectiveRpc.listenToEvents(handleEvent);
        if (disposed) {
          cleanup();
          return;
        }
        unlisten = cleanup;
        await effectiveRpc.openSession({ sessionId, target });
      } catch (error) {
        emitError(error);
      }
    })();
    ...
    const transport: DaemonTransport = {
      send: (data) => {
        ...
        if (typeof data === "string") {
          void effectiveRpc
            .sendMessage({ sessionId, text: data })
        ...
        void effectiveRpc
          .sendMessage({ sessionId, binaryBase64 })
          .catch((error: unknown) => emitError(error));
      },
      close: () => {
        if (disposed) {
        ...
        disposed = true;
        void effectiveRpc.closeSession(sessionId).catch((error: unknown) => emitError(error));
        unlisten?.();
        unlisten = null;
      },
      onMessage: (handler) => {
