...
function renderChatAgentNonReadyView(args: {
  ...
  if (viewState.tag === "error") {
    return <AgentLoadErrorView message={viewState.message} onRetry={onRetry} />;
  }
...
function AgentPanelBody({
  ...
  const _retryAgentLookup = useCallback(() => setLookupState({ tag: "idle" }), []);
  ...
  if (lookupState.tag === "error") {
    return <AgentLoadErrorView message={lookupState.message} onRetry={handleRetryLookup} />;
  }
...
const ChatAgentReadyContent = memo(function ChatAgentReadyContent({
  ...
  // Goal renders above the composer and needs the same transcript clearance as the track rail
  const hasVisibleGoalTrack = useSessionStore((state) => {
    const session = state.sessions[serverId];
    const trackedAgent = session?.agents.get(agentId) ?? session?.agentDetails.get(agentId);
    return shouldShowAgentGoalTrack({
  ...
  const handleEditLastUserMessageEffect = useCallback(
    ...
    [editLastUserMessageController, replaceText, resyncEditedTimeline, t, toastApi],
  );
  const isCompactFormFactor = useIsCompactFormFactor();
  const { workspaceId: paneWorkspaceId, tabId } = usePaneContext();
  const selectionOwnerId = `${serverId}:${paneWorkspaceId}:${tabId}`;
  ...
  const handleSelectionAsk = useCallback(
    (selected: string) => {
      ...
      replaceText(current.trim() ? `${current}\n${selected}` : selected);
      clearChatSelection();
    },
    [clearChatSelection, replaceText],
  );
    ...
    <View style={animatedStaticStyles.content}>
      <RenderProfile id={`AgentStreamSection:${agentId}`}>
        <AgentStreamSection
          ...
          hasVisibleComposerTracks={hasVisibleComposerTracks}
          toast={toastApi}
...
const AgentStreamSection = memo(function AgentStreamSection({
  ...
  hasAppliedAuthoritativeHistory,
  hasActiveComposer,
  hasVisibleComposerTracks: hasVisibleComposerTrackRail,
  toast,
  editLastUserMessageController,
  ...
  hasVisibleComposerTracks: boolean;
  ...
  const bottomOverlayTailClearance = hasVisibleBottomOverlayTracks
    ...
    : 0;
  const bottomOverlayControlClearance = hasVisibleBottomOverlayTracks
