...
// oxlint-disable-next-line complexity -- persisted agent snapshots keep compatibility fields together
...
function deserializeAgent(serverId: string, stored: StoredAgent): Agent {
  ...
  return {
    ...normalized,
    ...(normalized.goal ? { goalSync: "stale" as const } : {}),
    turn,
...
export class ReplicaCache {
  ...
  /** Stops pending final-state timers without dropping persisted rows */
  stop(): void {
  ...
  /** Opens the row store before directory restoration begins */
  async restore(): Promise<void> {
    await this.prepareStore();
    await this.ensureStoredIndex();
  }

  /** Retained for the host-runtime lifecycle; row-store observation is demand driven */
  start(): void {}
  ...
  /** Flushes all currently pending row changes */
  async flushDirty(): Promise<void> {
    await this.flush();
  }

  /** Flushes pending row changes after a focused-agent final-state signal */
  async flushDirtyHost(_serverId: string): Promise<void> {
    await this.flush();
  }

  /** Waits until coalesced row writes have become idle */
