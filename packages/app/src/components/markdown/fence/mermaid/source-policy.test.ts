...
describe("containsUnsafeMermaidSource", () => {
  it("rejects empty, oversized, directive, frontmatter, click, and external URI input", () => {
    expect(containsUnsafeMermaidSource(" \n\t ")).toBe(true);
    expect(containsUnsafeMermaidSource("x".repeat(100_001))).toBe(true);
    expect(containsUnsafeMermaidSource("%%{init: {}}%%\nflowchart TD\nA --> B")).toBe(true);
    expect(containsUnsafeMermaidSource("---\ntitle: Demo\n---\nflowchart TD\nA --> B")).toBe(true);
    expect(containsUnsafeMermaidSource("flowchart TD\nA --> B\nclick A callback")).toBe(true);
    expect(containsUnsafeMermaidSource("flowchart TD\nA[https://example.com]")).toBe(true);
    expect(containsUnsafeMermaidSource("flowchart TD\nA[javascript:alert(1)]")).toBe(true);
  });
