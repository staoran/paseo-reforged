...
export function containsUnsafeMermaidSource(code: string): boolean {
  if (
    code.length > MAX_MERMAID_SOURCE_LENGTH ||
    code.trim().length === 0 ||
    MERMAID_FRONTMATTER_PATTERN.test(code) ||
    MERMAID_DIRECTIVE_PATTERN.test(code) ||
    MERMAID_CLICK_PATTERN.test(code) ||
    EXTERNAL_URI_PATTERN.test(code)
  ) {
    return true;
  }
