 ...
 *
 * Everything below the trigger is the shared menu engine. Only the way it opens is different
...
function ContextMenuExclusiveOwner({ children }: PropsWithChildren): ReactNode {
  const { open, setOpen } = useMenuContext("ContextMenuExclusiveOwner");
...
export function ContextMenu({
  ...
  return (
    <MenuRoot {...props} compactMode={compactMode}>
      <ContextMenuExclusiveOwner>{children}</ContextMenuExclusiveOwner>
    </MenuRoot>
  );
}

export function ContextMenuContent(props: MenuSurfaceProps): ReactElement | null {
  return <MenuSurface {...props} allowWebContextMenuPassthrough />;
}
