...
export function MenuOverlay({
  ...
  const setWebOverlayNode = useCallback(
    (node: View | null) => {
      webOverlayRef.current = node as unknown as HTMLElement | null;
  ...
  useEffect(() => {
    if (!isWeb || !visible || !allowWebContextMenuPassthrough) return undefined;

    const isInsideMenu = (target: EventTarget | null) =>
      target instanceof Node && webOverlayRef.current?.contains(target) === true;
  ...
  }, [allowWebContextMenuPassthrough, onClose, visible]);

  if (!visible) return null;

  let webOverlayStyle: StyleProp<ViewStyle> = isWeb ? styles.overlayWeb : null;
  if (isWeb && allowWebContextMenuPassthrough) {
    webOverlayStyle = styles.overlayWebPassthrough;
  }
