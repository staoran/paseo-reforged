...
describe("control geometry", () => {
  ...
  it("keeps one size contract across buttons and segmented controls", () => {
    ...
    // Segments sit inside a track, so their horizontal padding is one tier tighter
    expect(geometry.segmentedSegmentXs.paddingHorizontal).toBe(theme.spacing[2]);
    expect(geometry.segmentedSegmentSm.paddingHorizontal).toBe(theme.spacing[2]);
    expect(geometry.segmentedSegmentMd.paddingHorizontal).toBe(theme.spacing[3]);
  });
});
