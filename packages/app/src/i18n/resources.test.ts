...
describe("translation resources", () => {
  ...
  it("includes Settings expansion keys for the Batch 3B migration", () => {
    ...
    expect(en.settings.host.orchestration.enableTools.title).toBe("Enable Paseo Reforged tools");
  ...
  it("includes desktop update utility keys for the Batch 4Q migration", () => {
    ...
    expect(en.desktop.rosetta.runningIntel).toBe(
      "You're running the Intel build of Paseo Reforged under Rosetta on Apple Silicon.",
    );
  ...
  it("includes desktop daemon settings keys for the Batch 4S migration", () => {
    ...
    expect(en.desktop.integrations.cli.installFailed).toBe(
      "Unable to install the Paseo Reforged CLI.",
    );
  });
