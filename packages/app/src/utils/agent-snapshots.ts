...
export function normalizeAgentSnapshot(snapshot: AgentSnapshotPayload, serverId: string) {
  ...
  return {
    ...
    providerRetryMessage: snapshot.providerRetryMessage ?? null,
    ...(snapshot.goal !== undefined ? { goal: snapshot.goal } : {}),
    ...(snapshot.goalStep !== undefined ? { goalStep: snapshot.goalStep } : {}),
    ...(snapshot.goalSync !== undefined ? { goalSync: snapshot.goalSync } : {}),
