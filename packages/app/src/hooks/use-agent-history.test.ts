...
import {
  ...
  resolveAgentHistorySearchAvailability,
...
describe("fetchAgentHistoryPage", () => {
  it("distinguishes supported, outdated, and not-yet-known host capabilities", () => {
    expect(resolveAgentHistorySearchAvailability([])).toEqual({
      isSearchSupported: false,
      isSearchUpgradeRequired: false,
    });
    expect(resolveAgentHistorySearchAvailability([true, true])).toEqual({
      isSearchSupported: true,
      isSearchUpgradeRequired: false,
    });
    expect(resolveAgentHistorySearchAvailability([true, false])).toEqual({
      isSearchSupported: false,
      isSearchUpgradeRequired: true,
    });
    expect(resolveAgentHistorySearchAvailability([true, null])).toEqual({
      isSearchSupported: false,
      isSearchUpgradeRequired: false,
    });
  });
