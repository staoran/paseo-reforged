...
describe("createWebStreamStrategy", () => {
  ...
  it("rerenders only the history row whose content revision changed", () => {
    ...
    const HistoryRow = React.memo(function HistoryRow({ row }: { row: StreamRenderRow }) {
      renderCountById.set(row.id, (renderCountById.get(row.id) ?? 0) + 1);
    ...
    const renderInput: StreamRenderInput = {
      ...
      renderers: {
        ...createRenderers(vi.fn()),
        renderHistoryMountedRow: (row) => <HistoryRow row={row} />,
      },
      listEmptyComponent: null,
      viewportRef,
      routeBottomAnchorRequest: null,
