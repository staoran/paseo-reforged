...
function strategyFor(platform: "web" | "native"): StreamStrategy {
  ...
  const getHistoryBoundaryIndex = (rows: readonly unknown[]) => {
    if (rows.length === 0) return null;
    return inverted ? 0 : rows.length - 1;
  };
  const getLiveHeadBoundaryIndex = (rows: readonly unknown[]) => {
    if (rows.length === 0) return null;
    return inverted ? rows.length - 1 : 0;
  };
  return {
...
function renderRows(items: StreamItem[]): StreamRenderRow[] {
  return items.map(createItemStreamRenderRow);
}
...
function layoutFor(input: { platform: "web" | "native"; isTurnActive: boolean }) {
  ...
  return {
    ...
    layout: layoutStream({
      ...
      history: strategy.orderTail(renderRows([userMessage("user"), assistant])),
      liveHead: [],
  ...
  it.each([
    ...
    (platform, isTurnActive) => {
      ...
      const layout = layoutStream({
        strategy,
        isTurnActive,
        history: strategy.orderTail(renderRows([userMessage("user"), first])),
        liveHead: strategy.orderHead(renderRows([last])),
