...
For provider installation, environment, or availability failures, run `paseo provider diagnostic <provider>`. The command asks the selected daemon for its diagnostic, so use the daemon's result rather than inferring availability from the client's local `PATH`.
